#!/bin/bash
# Daily Scan — runs via cron, invokes claude --print in non-interactive mode
# Platform: Claude Code

set -uo pipefail  # 注意：去掉了 -e，改为手动检查关键步骤的退出码

export PATH="$HOME/.local/bin:$PATH"
# claude 通过 nvm 安装，cron 环境不加载 nvm，需手动添加 node bin 到 PATH
...
which claude >> "$LOG_DIR/$DATE.env.log" 2>&1
which python3 >> "$LOG_DIR/$DATE.env.log" 2>&1

run_agent() {
    cd "$RADAR_HOME"
    timeout $TIMEOUT_DAILY claude --print \
...
# claude --print may exit 0 without producing output on API errors.
# Verify output file exists even on exit 0.
verify_output() {
...
if [ $EXIT_CODE -eq 0 ] && verify_output; then
    ...
    [ $EXIT_CODE -eq 0 ] && echo "[$DATE] Agent exited 0 but output missing, treating as failure." >> "$LOG_DIR/cron.log"

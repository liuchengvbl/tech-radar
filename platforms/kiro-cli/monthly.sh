...
run_agent() {
    cd "$RADAR_HOME"
    timeout $TIMEOUT_MONTHLY kiro-cli chat --no-interactive \
...
is_dispatch_failure() {
    grep -q "dispatch failure" "$LOG_DIR/monthly-$MONTH.log" 2>/dev/null
}
...
if [ $EXIT_CODE -eq 0 ] && verify_output && ! is_dispatch_failure; then
    ...
    while [ $attempt -lt $max_attempts ]; do
        ...
        if is_dispatch_failure; then
            echo "[$MONTH_END] Monthly $MONTH attempt $((attempt-1)) failed due to dispatch failure, retrying ($attempt/$max_attempts) in 30s..." >> "$LOG_DIR/cron.log"
            sleep 30
        else

...
本 agent 运行在 kiro-cli 环境，工具名称如下：
- 搜索网络内容：`web_search`（原生可用，支持时间范围过滤）
- 获取网页完整内容：`web_fetch`
- 写文件：`write`（支持 create 覆盖 + append 追加，每次调用 ≤150 行）
- 读文件：`read`
- 运行脚本（如 Python arxiv）：`shell`

写入策略：因 kiro write 工具每次调用限制 ≤150 行，需分段写入：
1. 第一次用 `create` 模式写入文件头
2. 后续用 `append` 模式逐段追加（每段 ≤150 行）
